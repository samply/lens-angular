...
export class MeasureTransformerService implements ResultTransformer {
  public siteToDefaultCollectionId: Map<string, string> = new Map<
    string,
    string
  >()
  ...
  transform(results: Map<string, any>): Array<Measure> {
    ...
    let patientsPerSite = new Stratifier('patients', []);
    results.forEach((unmappedData, requestTarget) => {
      let mappedData = unmappedData.body ? unmappedData.body : unmappedData;
      ...
      if (data != undefined && data.group instanceof Array) {
        ...
        console.debug(
          'Received empty dataset from site, adding empty measures for this site!'
        );
      ...
      siteMeasure.value += 1;
      let siteStratifier = new Stratifier(requestTarget, []);
      patientsPerSite.stratum = patientsPerSite.stratum.concat({
        key: requestTarget,
        population:
          data != undefined && data.group instanceof Array
            ? data.group[0].population[0].count
            : 0,
      });
  ...
  private transformToMeasure(group: any, requestTarget: string): Measure {
    let measure = new Measure(group.code.text, group.population[0].count, []);
    for (
      let stratifierIndex = 0;
      stratifierIndex < group.stratifier.length;
      stratifierIndex++
    ) {
      let title: string = group.stratifier[stratifierIndex].code[0].text;
      let stratifier = new Stratifier(title, []);
      ...
      for (
        let stratumIndex = 0;
        stratumIndex < group.stratifier[stratifierIndex].stratum.length;
        stratumIndex++
      ) {
        let key: string =
          group.stratifier[stratifierIndex].stratum[stratumIndex].value.text;
        ...
        stratifier.stratum = stratifier.stratum.concat({
          key: key,
          population:
            group.stratifier[stratifierIndex].stratum[stratumIndex]
              .population[0].count,
        });
      }
  ...
  public mergeMeasures(
    ...
    siteMeasures: Array<Measure>
  ): Array<Measure> {
    let mergedMeasures: Array<Measure> = measures;
    siteMeasures.forEach((transformedMeasure) => {
      let previousMeasureIndex = measures.findIndex(
        (measure) => measure.key == transformedMeasure.key
      );
      if (previousMeasureIndex > -1) {
        ...
        transformedMeasure.stratifier.forEach((transformedStratifier) => {
          let previousStratifierIndex = mergedMeasure.stratifier.findIndex(
            (stratifier) => stratifier.key == transformedStratifier.key
          );
          if (previousStratifierIndex > -1) {
            let mergedStratifier =
              mergedMeasure.stratifier[previousStratifierIndex];
            transformedStratifier.stratum.forEach((transformedStratum) => {
              let previousStratumIndex = mergedStratifier.stratum.findIndex(
                (stratum) => stratum.key == transformedStratum.key
              );
              if (previousStratumIndex > -1) {
                let mergedStratum =
                  mergedStratifier.stratum[previousStratumIndex];
